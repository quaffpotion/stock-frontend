...
export class AppComponent {
  ...
  preprocess(data) {
    data = data.map(x => {
      return { open: x.open, close: x.close, high: x.high, low: x.low };
    }); // deep copy
    const min = Math.min(...data.map(x => x.low));
    data.map(item => {
      item.open = item.open - min;
      item.close = item.close - min;
      item.high = item.high - min;
      item.low = item.low - min;
      return item;
    });
    return data;
  }
  ...
  getData() {
    this.mydata = this.iex
      .getSymbol(this.myStock.toUpperCase(), this.myNumber1, this.myNumber2)
      .pipe(map(data => this.preprocess(data)));
  }
  ...
  getData2() {
    return [this.generateOhlc(), this.generateOhlc(), this.generateOhlc()];
  }
}
